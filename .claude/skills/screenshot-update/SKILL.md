...
description: 'Webサイトに変更を加えたときに screenshot.spec.ts を実行してスクリーンショットを更新するスキル。UIの変更・新ページ追加・デザイン修正後に必ず /screenshot-update を実行する。コンポーネント実装・機能追加・スタイル変更・レイアウト修正など画面表示に影響する開発タスクを完了したときは必ず自動的に実行する。CSS/Tailwind の変更・React コンポーネントの追加や修正・新規ページルートの追加・表示ロジックの変更後にも必ず実行すること。「スクリーンショットを更新して」「スクショを撮って」「screenshot を再生成して」などの明示的な依頼にも使用する。'
---
